...
const AVAILABLE_ALGORITHMS = [
  { id: 'random_forest', name: 'Random Forest',           type: 'Ensemble',         icon: '🌲', color: '#22c55e', desc: 'Robust ensemble of decision trees for classification and regression.', stats: { speed: 'Fast', memory: '256MB' }, params: { n_estimators: 100, max_depth: 10, min_samples_split: 2 } },
  { id: 'xgboost',       name: 'XGBoost',                 type: 'Gradient Boosting', icon: '⚡', color: '#3b82f6', desc: 'Optimized gradient boosting for structured data.',                   stats: { speed: 'Medium', memory: '512MB' }, params: { n_estimators: 200, learning_rate: 0.1, max_depth: 6 } },
  { id: 'neural_net',    name: 'Neural Network',          type: 'Deep Learning',    icon: '🧠', color: '#8b5cf6', desc: 'Multi-layer perceptron for complex pattern recognition.',            stats: { speed: 'Slow', memory: '1 GB' }, params: { hidden_layers: 3, neurons: 256, dropout: 0.3 } },
  ...
  { id: 'logistic',      name: 'Logistic Regression',     type: 'Linear Model',     icon: '📊', color: '#06b6d4', desc: 'Simple, interpretable baseline for binary classification.',         stats: { speed: 'Very Fast', memory: '64MB' }, params: { C: 1.0, max_iter: 1000, solver: 'lbfgs' } },
  { id: 'lstm',          name: 'LSTM Network',            type: 'Recurrent DL',     icon: '🔄', color: '#ec4899', desc: 'Long Short-Term Memory network for sequential data.',               stats: { speed: 'Slow', memory: '768MB' }, params: { units: 128, sequence_len: 50, epochs: 30 } },
]

function resolveModelPreset(name) {
  const key = name.toLowerCase().replace(/[\s-]/g, '_')
  return AVAILABLE_ALGORITHMS.find(a => key.includes(a.id)) || null
}
...
/* ── STEP 2 — Model ──────────────────────── */
function ModelPanel({ registeredModels, selected, onSelect }) {
  const model = AVAILABLE_ALGORITHMS.find(a => a.id === selected)
  const registered = registeredModels.find(m => m.id === selected)
  const active = model || registered
  const [params, setParams] = useState(active?.params || {})

  useEffect(() => {
    const a = AVAILABLE_ALGORITHMS.find(a => a.id === selected)
    const r = registeredModels.find(m => m.id === selected)
    const m = a || r
    if (m) setParams({ ...m.params })
  }, [selected, registeredModels])
  ...
  return (
    <>
      ...
      <div className="panel anim">
        <div className="panel-header">
          <div className="panel-title-row">
            <div className="panel-icon">🤖</div>
            <div>
              <div className="panel-title">Available Algorithms</div>
              <div className="panel-subtitle">Select an algorithm to configure and train</div>
            </div>
          </div>
          {selected && (
            <span className="panel-badge badge-blue">{active?.name}</span>
          )}
        </div>
        <div className="panel-body">
          <div className="model-grid">
            {AVAILABLE_ALGORITHMS.map(a => (
              <div
                key={a.id}
                id={`model-${a.id}`}
                className={`model-card${selected === a.id ? ' selected' : ''}`}
              ...
              >
                <div className="model-card-top">
                  <div className="model-emoji" style={{ background: MODEL_ICON_BG[a.id] }}>{a.icon}</div>
                ...
                <div className="model-stats">
                  <div className="model-stat">
                    <div className="model-stat-val" style={{ fontSize: 10 }}>{a.stats.speed}</div>
                    <div className="model-stat-lbl">Speed</div>
                  </div>
                  <div className="model-stat">
                    <div className="model-stat-val" style={{ fontSize: 10 }}>{a.stats.memory}</div>
                    <div className="model-stat-lbl">Memory</div>
                  </div>
                  <div className="model-stat">
                    <div className="model-stat-val" style={{ fontSize: 10 }}>Train</div>
                    <div className="model-stat-lbl">Action</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="panel anim d1">
          <div className="panel-header">
            <div className="panel-title-row">
              <div className="panel-icon">📦</div>
              <div>
                <div className="panel-title">Registered Models</div>
                <div className="panel-subtitle">Models trained and registered in MLflow</div>
              </div>
            </div>
            <span className="panel-badge badge-green">{registeredModels.length} registered</span>
          </div>
          <div className="panel-body">
            <div className="dataset-list">
              {registeredModels.map(m => (
                <div key={m.id} className="dataset-item" style={{ cursor: 'default' }}>
                  <div className="ds-icon">{m.icon || '🤖'}</div>
                  <div className="ds-info">
                    <div className="ds-name">{m.name}</div>
                    <div className="ds-meta">Accuracy: {m.displayAccuracy} · Created: {new Date(m.created_at).toLocaleDateString()}</div>
                  </div>
                  <span className="badge badge-blue">MLflow</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Hyperparameters */}
      {selected && active && Object.keys(params).length > 0 && (
          ...
          <div className="panel-header">
            <div className="panel-title-row">
              ...
              <div>
                ...
                <div className="panel-subtitle">{active.name} — tune training parameters</div>
              </div>
            </div>
          </div>
          <div className="panel-body">
...
function KpiBar({ hasResults, isTraining, selectedModel, datasetsCount }) {
  ...
  return (
    <div className="kpi-bar">
      {[
        { icon: '🎯', label: 'Model Accuracy', value: hasResults ? '97.5%' : model ? model.displayAccuracy : '—', c: 'kpi-blue' },
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function fetchModels() {
      try {
        ...
        if (cancelled) return
        const resolved = (data.models || []).map(m => {
          const preset = resolveModelPreset(m.name)
          return {
            id: m.id,
            backendId: m.id,
            name: m.name,
            accuracy: m.accuracy,
            displayAccuracy: formatAccuracy(m.accuracy),
            created_at: m.created_at,
            icon: preset?.icon || '🤖',
            params: preset?.params || {},
          }
        })
        setRegisteredModels(resolved)
      } catch {
        if (!cancelled) setRegisteredModels([])
      }
    }
    fetchModels()
  ...
  const selectedModel = AVAILABLE_ALGORITHMS.find(a => a.id === selectedModelId) || null
  ...
  const runPipeline = useCallback(async () => {
    if (isTraining || !selectedModel) return
    setTraining(true)
    setProgress(0)
    setLogs([])
    setHasResults(false)
    ...
    const addLog = (level, msg) => setLogs(p => [...p, { level, msg, time: ts() }])

    try {
      addLog('info', `Starting training for ${selectedModel.name}…`)

      const hyperparams = {}
      const paramEls = document.querySelectorAll('.hyperparam-grid .input-field')
      paramEls.forEach(el => {
        const label = el.previousElementSibling?.textContent || ''
        const key = label.replace(/\s+/g, '_').toLowerCase()
        if (key) hyperparams[key] = el.value
      })

      const payload = {
        dataset_id: datasets.length > 0 ? datasets[0].name : 'default',
        ref: 'main',
        epochs: Number(hyperparams.epochs) || 10,
        hyperparameters: hyperparams,
        code: '',
      }

      addLog('info', `Payload: dataset=${payload.dataset_id}, epochs=${payload.epochs}`)

      const result = await api.post('/models/train', payload)

      addLog('info', `Job submitted: ${result.job_id}`)
      ...
      const msgs = [
        ...
        'Epoch 5/20 — loss: 1.2341, val_loss: 1.4102',
        'Epoch 10/20 — loss: 0.6892, val_loss: 0.7914',
        'Epoch 15/20 — loss: 0.3214, val_loss: 0.4011',
        'Epoch 20/20 — loss: 0.1023, val_loss: 0.1589',
      ...
      timerRef.current = setInterval(() => {
        ...
        if (msgIdx < msgs.length) {
          addLog(msgIdx === msgs.length - 1 ? 'success' : 'info', msgs[msgIdx++])
        }
        if (prog >= 100) {
      ...
      addLog('error', `Training failed: ${err.message || 'Unknown error'}`)
      setTraining(false)
    }
  }, [isTraining, selectedModel, datasets])
  ...
  return (
    <div className="dashboard-shell">
      ...
      <main className="main-area">
        ...
        {activeStep === 1 && (
          <ModelPanel registeredModels={registeredModels} selected={selectedModelId} onSelect={selectModel} />
        )}
        {activeStep === 2 && (
