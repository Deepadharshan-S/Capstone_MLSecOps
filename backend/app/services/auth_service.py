...
class AuthService:
    """
    Service class managing user registration, authentication session states,
    ...
    def authenticate_user(
        ...
        # Generic, identical response used for every "can't log this person in
        # right now" case below (nonexistent user, wrong password, locked
        # account). Keeping status code AND message identical across all
        # three prevents an attacker from using the response to enumerate
        ...
        if user.locked_until:
            ...
            if locked_until > current_time:
                ...
                # Same status/message as bad credentials -- see note above.
                # The account IS still locked (this request is rejected
                # either way); we just don't reveal that distinction.
                raise generic_credentials_error
        ...
        # secure=not settings.DEBUG so local HTTP dev (e.g. http://localhost)
        # still gets the cookie set; in production DEBUG=False so secure=True
        # as it should be (cookie only sent over HTTPS).
        response.set_cookie(
            key="refresh_token",
            value=refresh_token,
            httponly=True,
